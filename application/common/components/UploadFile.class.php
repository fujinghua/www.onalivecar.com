...
{//类定义开始
     ...
     * @access public
     +----------------------------------------------------------
     */
    public function __construct($maxSize='',$allowExts='',$allowTypes='',$savePath='',$saveRule='',$mysavename='')
    {
    ...
    public function upload($savePath ='')
    {
        ...
        // 获取上传的文件信息
        // 对$_FILES数组信息处理
		/*  if(!empty($this->filesName)){
			$files   =  $this->dealFiles($_FILES[$this->filesName]);
		}else{
			$files	 =	 $this->dealFiles($_FILES);
		} */
		$files	 =	 $this->dealFiles($_FILES);
		if(!empty($this->filesName)){
			$files   =  array($this->filesName=>$files[$this->filesName]);
		}
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
				...
				if($this->mysavename){
					$wt=explode(".",$file['savename']);
					$file['savename']=$this->mysavename.".".$wt[1];
				}
				
				//dump("底层提示：".$file['savename']);
                // 自动检查附件
                if($this->autoCheck) {
                    if(!$this->check($file))
                        return false;
				...
				$file['truesavepath']=$file['savepath'];//保存真实存储路径
				$file['savepath'] = preg_replace("/^\/data\/images\/wofang/","",$file['savepath']);//保存数据库存储路径
                $fileInfo[] = $file;
                $isUpload   = true;
            }
        }
