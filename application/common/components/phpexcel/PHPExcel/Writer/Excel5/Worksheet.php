...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	private function _writeSelection()
	{
		...
		list($colFirst, $rapp\common\componentsirst) = PHPExcel_Cell::coordinateFromString($first);
		$colFirst = PHPExcel_Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
		--$rapp\common\componentsirst; // base 0 row index
		...
		$rapp\common\componentsirst = min($rapp\common\componentsirst, 65535);
		...
		$rwAct	= $rapp\common\componentsirst;				// Active row
		$colAct   = $colFirst;			   // Active column
		$irefAct  = 0;					   // Active cell ref
		$cref	 = 1;					   // Number of refs

		if (!isset($rwLast)) {
			$rwLast   = $rapp\common\componentsirst;	   // Last  row in reference
		}
		if (!isset($colLast)) {
		...
		if ($rapp\common\componentsirst > $rwLast) {
			list($rapp\common\componentsirst, $rwLast) = array($rwLast, $rapp\common\componentsirst);
		}
		...
		$data	 = pack("CvvvvvvCC",  $pnn, $rwAct, $colAct,
									   $irefAct, $cref,
									   $rapp\common\componentsirst, $rwLast,
									   $colFirst, $colLast);
		$this->_append($header . $data);
	}
