...
class Slider extends Model
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'type' => '父级类型:1=默认,2=首页,3=新房,4=二手房,5=出租,6=,7=联系我们,;默认1;',
            'url' => '图片地址',
            'url_icon' => '图片略缩图',
            'target' => '跳转地址',
            'title' => '标题',
            ...
            'isDefault' => '是否是默认图片,1为是，2为否，默认为2',
            'created_at' => '创建时间',
            'updated_at' => '修改时间',
        ];
    }
     ...
     * @return array
     */
    public static function getSlider($where = [],$limit = 6,$isDefault = false){
        $ret = [];
        $limit = $limit ? $limit : 6;
        if (!empty($where)){
            $time = "'".date('Y-m-d')."'";
            ...
            $model = Slider::load();
            $ret = $model->asArray($model->where($where)->order('`order` ASC')->limit($limit)->select());
        }
        return $ret;
    }
