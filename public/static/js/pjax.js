...
layui.define(['layer', 'form'], function (exports) {
     ...
     * 版本号
     */
    Pjax.prototype.v = '1.0.0';

    /**
     * 设置
     * @param {Object} options
     */
    Pjax.prototype.set = function (options) {
        var that = this;
     ...
     * 渲染
     */
    Pjax.prototype.render = function () {
        var that = this;
        var _config = that.config;
        ELEM.parent = $(_config.elem);
        ELEM.toggle = $(_config.toggleElem);
        if (ELEM.parent === 0) {
            layer.msg('Pjax Error:找不到配置的容器elem!');
            ELEM.valid = false;
        }
        if (ELEM.toggle.length === 0) {
            layer.msg('Pjax Error:找不到配置的容器toggleElem!');
            ELEM.valid = false;
        }
        if (ELEM.valid){
            if (_config.type !== 2){
    ...
    /**
     * 查询Pjax 标题 是否存在，如果存在则返回索引值，不存在返回-1
     * @param {String} title 标题
     * @param {String} id  参数filter设置为 2 才生效
     */
    Pjax.prototype.exists = function(title,id) {
     ...
     * @param {String} id  参数filter设置为 2 才生效
     */
    Pjax.prototype.getTitle=function(title,id){
        var _config = this.config;
        var that = ELEM.titleBox === undefined ? this.init() : this,
            tabId = -1;
        if ( parseInt(_config.filter) !== 2 ){
    ...
    Pjax.prototype.on = function(events, callback) {
    };
