...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  //主模板
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}};{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}};{{# } }}">'
    ,'{{# var left, right; }}'
    ...
    ,'{{# layui.each(d.data.cols, function(i1, item1){'
      ,'layui.each(item1, function(i2, item2){ }}'
        ,'.laytable-cell-{{d.index}}-{{item2.field||i2}}{ width:{{item2.width}} }'
      ,'{{# });'
    ,'}); }}'
    ,'</style>'
  ,'</div>'].join('')
